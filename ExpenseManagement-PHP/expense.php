<?php  
  class Expense extends Base {
    ...
    public function getMonthlyExpensesByCategory($UserId) {
      $stmt = $this->pdo->prepare("SELECT Category, SUM(Cost) AS Total FROM expense WHERE UserId = :UserId AND MONTH(Date) = MONTH(CURRENT_DATE()) AND YEAR(Date) = YEAR(CURRENT_DATE()) GROUP BY Category");
      $stmt->bindParam(":UserId", $UserId, PDO::PARAM_INT);
      $stmt->execute();
